...
from app.schemas.analytics import  AIPerformanceResponse, ChannelAnalyticsResponse, ResponseTimeResponse
from app.core.database import get_db
from app.core.security import require_admin
from app.schemas.analytics import AnalyticsDateRange,AnalyticsOverviewResponse,ConversationAnalyticsResponse,MAX_DATE_RANGE_DAYS
...
async def get_analytics_conversations(
    ...
    result = await service.get_conversation_analytics(
        organization_id=current_user.organization_id,
        start_date=start_date,
        end_date=end_date,
    )
...
async def get_analytics_ai_performance(
    ...
    result = await service.get_ai_performance(
        organization_id=current_user.organization_id,
        start_date=start_date,
        end_date=end_date,
    )
...
async def get_analytics_channels(
    ...
    result = await service.get_channel_analytics(
        organization_id=current_user.organization_id,
        start_date=start_date,
        end_date=end_date,
    )
