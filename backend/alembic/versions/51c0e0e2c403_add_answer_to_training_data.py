...
def upgrade() -> None:
    """Upgrade schema."""
    op.add_column('training_data', sa.Column('answer', sa.Text(), nullable=False))
    op.alter_column('training_data', 'question',
               existing_type=sa.TEXT(),
               type_=sa.String(length=500),
               existing_nullable=False)
    op.alter_column('training_data', 'category',
...
def downgrade() -> None:
    """Downgrade schema."""
    op.add_column('training_data', sa.Column('created_at', sa.DateTime(), nullable=False))
    op.add_column('training_data', sa.Column('verified', sa.Boolean(), nullable=False))
