...
=======
if africastalking is not None and settings.AT_USERNAME and settings.AT_API_KEY:
    try:
...
=======
            logger.error(
                "SMS delivery is unavailable: africastalking library is missing or credentials are not configured"
            )
            return False

        try:
            # Reformat Tanzanian local numbers to E.164 format
            if to.startswith("0"):
                to = "+255" + to[1:]
